...
void push_char(context* ctx, const driver* drv, char c) {
    ...
    case State_0: {
        switch (c) {
        case 'd': ctx->state = State_105; break;
    ...
    case State_105: {
        switch (c) {
        case 'p': ctx->state = State_106; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    ...
    case State_107: {
        switch (c) {
        case ' ': ctx->state = State_108; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_108: {
        switch (c) {
        case 'b': ctx->state = State_181; break;
        case 'e': ctx->state = State_145; break;
        case 'l': ctx->state = State_109; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_109: {
        switch (c) {
        case 'o': ctx->state = State_110; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_110: {
        switch (c) {
        case 'x': ctx->state = State_111; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_111: {
        switch (c) {
        case ' ': ctx->state = State_112; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_112: {
        switch (c) {
        case 'a': ctx->state = State_124; break;
        case 'p': ctx->state = State_113; break;
        case 'r': ctx->state = State_139; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_113: {
        ...
        case 'r': ctx->state = State_114; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_114: {
        switch (c) {
        case 'e': ctx->state = State_115; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_115: {
        switch (c) {
        case 's': ctx->state = State_116; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_116: {
        switch (c) {
        case 's': ctx->state = State_117; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_117: {
        switch (c) {
        case 'u': ctx->state = State_118; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_118: {
        switch (c) {
        case 'r': ctx->state = State_119; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    ...
    case State_120: {
        switch (c) {
        case 'z': ctx->state = State_121; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_121: {
        switch (c) {
        case 'e': ctx->state = State_122; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_122: {
        switch (c) {
        case ' ': ctx->state = State_123; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    ...
    case State_124: {
        switch (c) {
        case 'b': ctx->state = State_125; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_125: {
        switch (c) {
        case 'o': ctx->state = State_126; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_126: {
        switch (c) {
        case 'r': ctx->state = State_127; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_127: {
        switch (c) {
        case 't': ctx->state = State_128; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    ...
    case State_130: {
        switch (c) {
        case 's': ctx->state = State_131; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_131: {
        switch (c) {
        case 's': ctx->state = State_132; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_132: {
        switch (c) {
        case 'i': ctx->state = State_133; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_133: {
        switch (c) {
        case 'v': ctx->state = State_134; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_134: {
        switch (c) {
        case 'a': ctx->state = State_135; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_135: {
        switch (c) {
        case 't': ctx->state = State_136; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_136: {
        switch (c) {
        case 'e': ctx->state = State_137; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_137: {
        switch (c) {
        case ' ': ctx->state = State_138; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    ...
    case State_140: {
        switch (c) {
        case 's': ctx->state = State_141; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_141: {
        switch (c) {
        case 'e': ctx->state = State_142; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_142: {
        switch (c) {
        case 't': ctx->state = State_143; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    ...
    case State_145: {
        switch (c) {
        case 't': ctx->state = State_146; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_146: {
        switch (c) {
        case 'h': ctx->state = State_147; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_147: {
        switch (c) {
        case ' ': ctx->state = State_148; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_148: {
        switch (c) {
        case 'a': ctx->state = State_160; break;
        case 'p': ctx->state = State_149; break;
        case 'r': ctx->state = State_175; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_149: {
        ...
        case 'r': ctx->state = State_150; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    ...
    case State_151: {
        switch (c) {
        case 's': ctx->state = State_152; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_152: {
        switch (c) {
        case 's': ctx->state = State_153; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_153: {
        switch (c) {
        case 'u': ctx->state = State_154; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    ...
    case State_155: {
        switch (c) {
        case 'i': ctx->state = State_156; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_156: {
        switch (c) {
        case 'z': ctx->state = State_157; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_157: {
        switch (c) {
        case 'e': ctx->state = State_158; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    ...
    case State_160: {
        switch (c) {
        case 'b': ctx->state = State_161; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_161: {
        switch (c) {
        case 'o': ctx->state = State_162; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_162: {
        switch (c) {
        case 'r': ctx->state = State_163; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_163: {
        switch (c) {
        case 't': ctx->state = State_164; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    ...
    case State_168: {
        switch (c) {
        case 'i': ctx->state = State_169; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_169: {
        switch (c) {
        case 'v': ctx->state = State_170; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_170: {
        switch (c) {
        case 'a': ctx->state = State_171; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_171: {
        switch (c) {
        case 't': ctx->state = State_172; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    ...
    case State_176: {
        switch (c) {
        case 's': ctx->state = State_177; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_177: {
        switch (c) {
        case 'e': ctx->state = State_178; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    ...
    case State_181: {
        switch (c) {
        case 'r': ctx->state = State_182; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_182: {
        switch (c) {
        case 'o': ctx->state = State_183; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_183: {
        switch (c) {
        case 'a': ctx->state = State_184; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_184: {
        switch (c) {
        case 'd': ctx->state = State_185; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_185: {
        switch (c) {
        case 'c': ctx->state = State_186; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    ...
    case State_188: {
        switch (c) {
        case 't': ctx->state = State_189; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_189: {
        switch (c) {
        case '_': ctx->state = State_190; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_190: {
        switch (c) {
        case 'a': ctx->state = State_191; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_191: {
        switch (c) {
        case 'b': ctx->state = State_192; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_192: {
        switch (c) {
        case 'o': ctx->state = State_193; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
    case State_193: {
        switch (c) {
        case 'r': ctx->state = State_194; break;
        default: ctx->state = State_0; break;
        }
        break;
    }
